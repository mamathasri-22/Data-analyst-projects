...
SELECT 'movie_row' AS TABLE_NAME,
    COUNT(*) AS count
FROM 
    movie
UNION ALL
...
FROM 
    ratings
UNION ALL
 SELECT 'genre_row' AS TABLE_NAME ,
    COUNT(*)
FROM
    genre
UNION ALL
 SELECT 'director' AS TABLE_NAME ,
    COUNT(*)
FROM
    director_mapping
UNION ALL   
 SELECT 'role_star' AS Role, 
    COUNT(*)
FROM
    role_mapping
UNION ALL
 SELECT 'name_change' AS Name, 
    COUNT(*)
FROM
    names;
...
-- Type your code below:
SELECT 'title' AS count_null,COUNT(*) AS num
FROM movie WHERE title IS NULL
UNION ALL
SELECT 'year' AS count_null , COUNT(*) AS num
FROM movie WHERE year IS NULL
UNION ALL
SELECT 'date_published' AS count_null , COUNT(*) AS num
FROM movie WHERE date_published IS NULL
UNION ALL
SELECT 'duration' AS count_null , COUNT(*) AS num
FROM movie WHERE duration IS NULL
UNION ALL
SELECT 'country' AS count_null , COUNT(*) AS num
FROM movie WHERE country IS NULL
UNION ALL
SELECT 'worlwide_gross_income' AS count_null , COUNT(*) AS num
FROM movie WHERE worlwide_gross_income IS NULL
UNION ALL
SELECT 'languages' AS count_null , COUNT(*) AS num
FROM movie WHERE languages IS NULL
UNION ALL
SELECT 'production_company' AS count_null , COUNT(*) AS num
...
row_number() OVER(ORDER BY COUNT(m.title) DESC) AS prod_company_rank
FROM movie AS m 
...
SELECT 
    ROUND(german_movie_votes / german_movie_count) AS german_votes_per_movie,
    ROUND(italian_movie_votes / italian_movie_count) AS italian_votes_per_movie
FROM
    votes_summary;
...
SELECT 
	...
	/ SUM(total_votes) ,2) AS avg_rating,
